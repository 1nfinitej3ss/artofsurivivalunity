...
namespace Assets.PixelFantasy.PixelMonsters.Common.Scripts.ExampleScripts
{
    ...
    public class MonsterController2D : MonoBehaviour
    {
        ...
        public float localGravity = 9.8f; // Similar to PlayerController's local gravity
        ...
        public void Start()
        {
            ...
            // Initialize the Rigidbody2D settings
            _rigidbody.gravityScale = 0; // Disable default gravity
            _rigidbody.freezeRotation = true; // Prevent rotation

            // Set the WhatIsGround by default to Ground if needed
            // whatIsGround = 1 << LayerMask.NameToLayer("Ground");
        }

        public void Update()
        {
            IsGrounded = Physics2D.OverlapCircle(GroundCheck.position, 0.16f, whatIsGround);

            // Additional logic to check animation states, if needed
        }
        ...
        private void AlignToPlanet(Vector3 targetPosition)
        {
            ...
            _rigidbody.AddForce(toCenter * localGravity, ForceMode2D.Force);
